...
class AnalisisInfa:
    ...
    def capturar_utc_offset(self):
        ...
        zonas_utm_validas = ["-3", "-4"]
        if texto in zonas_utm_validas:
            if texto == "-3":
                self.utc_offset = 3
            elif texto == "-4":
                self.utc_offset = 4
    ...
    def procesar(self):
        ...
        capa_ccaa = self.dlg.ccaaFileWidget.filePath()
        ...
        if not capa_batimetria.endswith('.shp'):
            ...
            return
        if not capa_ccaa.endswith('.kmz'):
            self.mensajes_texto_plugin("Error: La capa CCAA debe tener la extensión .kmz")
            return

        # Llamar a las funciones
        self.mensajes_texto_plugin("Iniciando procesamiento de datos")
        self.procesar_ccaa(capa_ccaa, excel_path, directorio_salida_shp, directorio_salida_kmz)
    ...
    def procesar_ccaa(self, capa_ccaa, excel_path, directorio_salida_shp, directorio_salida_kmz):
        ccaa.ccaa_a_shp_kmz(capa_ccaa, excel_path, directorio_salida_shp, directorio_salida_kmz, self)
    ...
    def resetear_campos(self):
        ...
        self.dlg.ccaaFileWidget.setFilePath('')
