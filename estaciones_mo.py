...
def estaciones_mo(archivo_excel, directorio_salida_shp, directorio_salida_kmz, plugin_instance):
    """
    Función para extraer las estaciones de MO del archivo de datos Excel
    Inputs:
    ...
    df = pd.read_excel(file_path, sheet_name='Estaciones_MO')

    # Snippet de código para salir de la función en el caso de que las hoja de transectas esté vacía
    if df.empty:
        mensaje = f"\nNo hay estaciones de MO en el archivo Excel. Si esto es un error revisar archivo"
    ...
    fields.append(QgsField("Hora_Ini", QVariant.Time, "", 50))
    fields.append(QgsField("Hora_Ter", QVariant.Time, "", 50))
    fields.append(QgsField("Estacion", QVariant.String, "", 50))
    fields.append(QgsField("Tipo", QVariant.String, "", 50))
    ...
    QgsVectorFileWriter.writeAsVectorFormatV3(vlayer,
                                              f"{directorio_salida_shp}{nombre_archivo} Estaciones MO Ptos.shp",
                                              transform_context,
                                              options)
    ...
    for centro in df['Centro'].unique():
        ...
        for estacion in centro_df['Estacion'].unique():
            ...
            placemarks += placemark_template.format(
                ...
                Hora_Ini=estacion_df.iloc[0]['Hora_Ini'],
                Hora_Ter=estacion_df.iloc[0]['Hora_Ter'],
    ...
    path_icono = os.path.join(plugin_dir, 'files', 'icono_mo.png') # Refactorizado para plugin
    nombre = nombrar_archivo(archivo_excel)
    nombre_archivo = f"{nombre} Estaciones MO Ptos"
    ...
    with open(f"{directorio_salida_kmz}{nombre_archivo}.kml", 'w') as file: # Esto es temporal!!!
